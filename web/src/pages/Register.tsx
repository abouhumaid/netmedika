...
export default function Register() {
  const [values, setValues] = useState<RegisterFormValues>({ username: '', email: '', password: '' })
  ...
  function updateField<K extends keyof RegisterFormValues>(field: K, value: RegisterFormValues[K]) {
    ...
    setErrors((prev) => ({ ...prev, [field]: getFieldErrors(registerSchema, nextValues)[field] }))
  }

  async function submit(event: React.FormEvent) {
    event.preventDefault()
    const nextErrors = getFieldErrors(registerSchema, values)
    ...
    <div className="flex min-h-screen items-center justify-center px-4 py-10 sm:px-6">
      <section className="relative mx-auto w-full max-w-md overflow-hidden rounded-[32px] border border-white/60 bg-white/45 p-8 shadow-2xl shadow-teal-950/10 backdrop-blur-md sm:p-10">
        ...
        <div className="items-center gap-2 text-center">
          <p className="text-[15px] font-semibold uppercase tracking-[3px] text-teal-700">Join</p>
        ...
        <form onSubmit={submit} className="mt-8 space-y-5">
          <label className="block">
            ...
            <input
              ...
              required
            />
            {errors.username ? <span className="mt-2 block text-sm font-medium text-rose-600">{errors.username}</span> : null}
          </label>
