...
def _safe_delete_image(image_path_str: str) -> None:
    try:
        Path(image_path_str).unlink()
        logger.info("Deleted image: %s", image_path_str)
    except FileNotFoundError:
        pass
    except Exception as exc:
        logger.warning("Delete failed %s: %s", image_path_str, exc)
...
async def create_order(
    ...
    if not medicine_name and not uploaded_image:
        raise HTTPException(status_code=400, detail="Provide medicine name or image.")
    ...
    try:
        ...
        new_order = Order(
            order_id=order_id,
            user_id=current_user.id,   # ✅ ALWAYS INT
            dosage_form=dosage_form,
            medication_name=medicine_name,
            strength=strength,
            frequency=frequency,
        ...
        db.rollback()
        logger.error("Create order failed: %s", exc, exc_info=True)
